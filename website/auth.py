...
def strat():
    if current_user.cname == "Kalibo":
        ...
        statss = Samplestrategies \
            ...
            .filter(User.id==current_user.id).count()
        print(statss)
        return render_template("sstrategies.html", user=current_user, statc=statc, statss=statss)
    else:
        if request.method == 'POST':
            name = request.form['name']
            act = request.form['act']
...
def newstrat():
    if current_user.cname == "Kalibo":
        if request.method == 'POST':
            ...
            flash("Strategy Inserted Successfully")
            statc = Samplestrategies \
                .query \
                .join(User) \
                .filter(Samplestrategies.status == "complete") \
...
def deletestrat(id):
    if current_user.cname == "Kalibo":
        ...
        statss = Samplestrategies \
            ...
            .filter(User.id==current_user.id).count()
        print(statss)
        return render_template("strategies.html", user=current_user, statc=statc, statss=statss)
    else:
        my_data = Samplestrategies.query.get(id)
        db.session.delete(my_data)
        db.session.commit()
