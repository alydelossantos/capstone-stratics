...
def importcsv():
    if current_user.cname == "Kalibo Cable":
        if request.method == 'POST':
            current_user.csv = request.files['csv']
            db.session.commit()
            ...
            records = pd.read_csv("https://raw.githubusercontent.com/alydelossantos/capstone-stratics/main/website/static/file/" + current_user.csv + "?token=AUEFNOE5ECSSQQFM272MQ73BT7QQK", names=col, header=0)
